...
//    }
    ...
    @Override
    public void reward(Player plKill) {
        ItemMap it = new ItemMap(this.zone, 1998, 50, this.location.x, this.zone.map.yPhysicInTop(this.location.x,
                this.location.y - 24), plKill.id);
//        it.options.add(new Item.ItemOption(73, 1));
    ...
    public void active() {
        ...
        if (Util.getDistance(playerTarger, this) > 500 && this.zone == this.playerTarger.zone) {
            Service.gI().sendThongBao(this.playerTarger, "Đi quá xa ,  Mị nương  đã rời đi ! ");
            playerTarger.haveDuongTang = false;
            this.leaveMap();
        }
        ...
        if (this.playerTarger != null && playerTarger.haveDuongTang && this.zone.map.mapId == this.mapCongDuc) { // xử lý khi đến map muốn đến
         ...
         Item honLinhThu1 = ItemService.gI().createNewItem((short) (861));
         int hongngoc = Util.nextInt(100,500);
            honLinhThu1.quantity = hongngoc;
//            playerTarger.inventory.ruby+=1000;
            Item honLinhThu = ItemService.gI().createNewItem((short) (457));
            int thoivang = Util.nextInt(1,5);
            honLinhThu.quantity = thoivang;
            //playerTarger.inventory.gem += 100000;
            Service.getInstance().sendMoney(playerTarger);
            InventoryServiceNew.gI().addItemBag(playerTarger, honLinhThu);
           Service.getInstance().sendThongBaoOK(playerTarger, "Bạn nhận được "+hongngoc+" hồng ngọc, "+thoivang+" thỏi vàng !");
            this.leaveMap();
        }
