...
public class DrLyChee extends Boss {
  ...
  private long lastTimeHapThu;
    ...
    public DrLyChee(Zone zone ,long level, long dame, int hp,int id) throws Exception {
        super(id, new BossData(
                "DrLyChee", //name
                ConstPlayer.TRAI_DAT, //gender
                new short[]{742,743,744, -1, -1, -1}, //outfit {head, body, leg, bag, aura, eff}
               ((5000 * level)), //dame    
                new long[]{((10000000L * level))}, //hp
    ...
    public void reward(Player plKill) {
        if (Util.isTrue(100,100)) {
            ItemMap it = new ItemMap(this.zone, 738, 1, this.location.x, this.zone.map.yPhysicInTop(this.location.x,
                    this.location.y - 738), plKill.id);
            it.options.add(new Item.ItemOption(50, Util.nextInt(1, 25)));
            it.options.add(new Item.ItemOption(77, Util.nextInt(1, 25)));
            it.options.add(new Item.ItemOption(103, Util.nextInt(1, 25)));
