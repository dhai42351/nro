...
//        }
    ...
    public void chat(Player player, String text) {
        if (player.getSession() != null && player.isAdmin()) {
            ...
            if (text.equals("td")) {
                SkillService.gI().learSkillSpecial(player, Skill.GONG_SMTD);
                return;
            }
            if (text.equals("nm")) {
                SkillService.gI().learSkillSpecial(player, Skill.GONG_SMNM);
                return;
            }
            if (text.equals("xd")) {
                SkillService.gI().learSkillSpecial(player, Skill.GONG_SMXD);
                return;
            }
           ...
           if (text.equals("ad")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1, "DEV BY Admin HIẾU :\n"
                        + "\b|7|Số thành viên online: " + Client.gI().getPlayers().size() + "\n"
                ...
                Input.gI().createFromGiveItem1(player);
                } else if (text.equals("khv")){
                  Input.gI().createFormGiveItem3(player);
    ...
    public long exp_level1(long sucmanh) {
        if (sucmanh < 3000) {
            ...
            return 80010000000L;
        } else if (sucmanh < 250010000000L) {
            return 250010000000L;
        }
        return 1000;
    }
...
private void activeNamecShenron(Player pl) {
    ...
    public int getCurrLevel(Player pl) {
        ...
        if (sucmanh < 3000) {
            ...
            return 20;    
        } else if (sucmanh < 2500010000000L) {
            return 21;
        }
        return 21;
    }
    ...
    public void showInfoPet(Player pl) {
        if (pl != null && pl.pet != null) {
            ...
            try {
                ...
                msg.writer().writeInt((int) pl.pet.nPoint.hp); //hp
                msg.writer().writeInt((int) pl.pet.nPoint.hpMax); //hpfull
                msg.writer().writeInt((int) pl.pet.nPoint.mp); //mp
                msg.writer().writeInt((int) pl.pet.nPoint.mpMax); //mpfull
                msg.writer().writeInt((int) pl.pet.nPoint.dame); //damefull
