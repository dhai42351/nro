...
public class EffectSkillService {
    ...
    // biến thành binh
    public void sendPlayerToCaiBinh(Player player, Mob mob, int timeBinh) {
        Message msg;
        try {
            msg = new Message(-112);
            msg.writer().writeByte(0);
            msg.writer().writeByte(mob.id); //mob id
            msg.writer().writeShort(11175); //icon binh
            Service.gI().sendMessAllPlayerInMap(player, msg);
            msg.cleanup();
            mob.effectSkill.SetHoaBinh(System.currentTimeMillis(), timeBinh);
        } catch (Exception e) {
            com.girlkun.utils.Logger.logException(EffectSkillService.class, e);
        }
    }
     //player biến thành binh
    public void setBinh(Player player, long lastTimeHoaBinh, int timeBinh) {
        player.effectSkill.lastTimeHoaBinh = lastTimeHoaBinh;
    ...
    //player trở lại thành người
    public void removeCaiBinh(Player player) {
    ...
    public void setStartHuytSao(Player player, long tiLeHP) {
        player.effectSkill.tiLeHPHuytSao = (int) tiLeHP;
    ...
    public void sendEffectMonkey(Player player) {
     ...
     public void sendEffectMonkey0(Player player) {
        Skill skill = SkillUtil.getSkillbyId(player, Skill.GONG_SMTD);
        Message msg;
        try {
            msg = new Message(-45);
            msg.writer().writeByte(5);
      ...
      public void sendEffectMonkey1(Player player) {
        Skill skill = SkillUtil.getSkillbyId(player, Skill.GONG_SMNM);
        Message msg;
        try {
            msg = new Message(-45);
            msg.writer().writeByte(5);
       ...
       public void sendEffectMonkey2(Player player) {
        Skill skill = SkillUtil.getSkillbyId(player, Skill.GONG_SMXD);
        Message msg;
        try {
            msg = new Message(-45);
            msg.writer().writeByte(5);
      ...
      void sendMobTomaphongba(Player playerMap, Mob mob, int timeBinh) {
        Message msg;
        try {
            msg = new Message(-112);
            msg.writer().writeByte(1);
            msg.writer().writeByte(mob.id); //mob id
            msg.writer().writeShort(11175); //icon socola
            Service.gI().sendMessAllPlayerInMap(playerMap, msg);
