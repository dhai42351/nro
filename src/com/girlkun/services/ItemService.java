...
public Item otptl(short tempId, int quantity) {
    ...
    public void OpenItem736(Player player, Item itemUse) {
     ...
     public void OpenItem1355(Player player, Item itemUse) {
        try {
            if (InventoryServiceNew.gI().getCountEmptyBag(player) <= 1) {
                Service.gI().sendThongBao(player, "Bạn phải có ít nhất 2 ô trống hành trang");
                return;
    ...
    public void OpenItem1356(Player player, Item itemUse) {
        try {
            if (InventoryServiceNew.gI().getCountEmptyBag(player) <= 1) {
                Service.gI().sendThongBao(player, "Bạn phải có ít nhất 2 ô trống hành trang");
                return;
            ...
            if (rd <= rac) {
                ...
                item = vanBay2011vv(true);
            } else if (rd <= rac + ruby + dbv + vb + bh) {
                item = phuKien2011vv(true);
            } else if (rd <= rac + ruby + dbv + vb + bh + ct) {
                item = caitrang2011(true);
            }
            if (item.template.id == 861) {
                item.quantity = Util.nextInt(10, 300);
            }
            icon[0] = itemUse.template.iconID;
            icon[1] = item.template.iconID;
            InventoryServiceNew.gI().subQuantityItemsBag(player, itemUse, 1);
...
public void settlkaio(Player player) throws Exception {
        for (int i = 0 ; i < 12;i++){
        Item hq = InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1269+i);
...
//        nhan.itemOptions.add(new Item.ItemOption(30,0));
    ...
    public Item phuKien2011(boolean rating) {
     ...
     public Item phuKien2011vv(boolean rating) {
        Item item = createItemSetKichHoat(954, 1);
        item.itemOptions.add(new Item.ItemOption(77, new Random().nextInt(5) + 10));
        item.itemOptions.add(new Item.ItemOption(103, new Random().nextInt(5) + 10));
        item.itemOptions.add(new Item.ItemOption(147, new Random().nextInt(5) + 10));
        if (Util.isTrue(1, 100)) {
            item.itemOptions.get(Util.nextInt(item.itemOptions.size() - 1)).param = 10;
        }
        item.itemOptions.add(new Item.ItemOption(30, 1));//ko the gd
...
//        }
    ...
    public Item vanBay2011(boolean rating) {
        ...
        item.itemOptions.add(new Item.ItemOption(89,  Util.nextInt(5,10)));//hp 
        item.itemOptions.add(new Item.ItemOption(50,  Util.nextInt(5,10)));//hp 
        item.itemOptions.add(new Item.ItemOption(77,  Util.nextInt(5,10)));//hp
        item.itemOptions.add(new Item.ItemOption(30, 1));//ko the gd
    ...
    public Item vanBay2011vv(boolean rating) {
        Item item = createItemSetKichHoat(795, 1);
        item.itemOptions.add(new Item.ItemOption(89,  Util.nextInt(8,15)));//hp 
        item.itemOptions.add(new Item.ItemOption(50,  Util.nextInt(8,15)));//hp 
        item.itemOptions.add(new Item.ItemOption(77,  Util.nextInt(8,15)));//hp 
...
public Item caitrang1355(boolean rating) {
        Item item = createItemSetKichHoat(464, 1);
        item.itemOptions.add(new Item.ItemOption(76, 1));//VIP
        item.itemOptions.add(new Item.ItemOption(77, Util.nextInt(18,35)));//hp 
        item.itemOptions.add(new Item.ItemOption(103, Util.nextInt(18,35)));//ki 
        item.itemOptions.add(new Item.ItemOption(147, Util.nextInt(18,35)));//sd 
        item.itemOptions.add(new Item.ItemOption(117, Util.nextInt(5,14)));//Đẹp + 18% sd
        if (Util.isTrue(995, 1000) && rating) {// tỉ lệ ra hsd
            item.itemOptions.add(new Item.ItemOption(93, new Random().nextInt(3) + 1));//hsd
        }
        return item;
    ...
    public Item randomRac() {
     ...
     public Item randomRac1() {
        short[] racs = {1099,1100,1101,1102};
        Item item = createItemSetKichHoat(racs[Util.nextInt(racs.length - 1)], 1);
        if (optionRac(item.template.id) != 0) {
            item.itemOptions.add(new Item.ItemOption(optionRac(item.template.id), 1));
        }
