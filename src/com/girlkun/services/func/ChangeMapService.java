        ...
        try {
            ...
            switch (pl.iDMark.getTypeChangeMap()) {
                ...
                case ConstMap.CHANGE_MAP_MA_BU:
                    ...
                    break;
                     case ConstMap.CHANGE_MAP_MA_BU2H:
                    list = (pl.mapMaBu2h != null ? pl.mapMaBu2h
                            : (pl.mapMaBu2h = MapService.gI().getMapMaBu2h()));
                    msg.writer().writeByte(list.size());
                    for (Zone zone : list) {
                        msg.writer().writeUTF(zone.map.mapName);
                        msg.writer().writeUTF(zone.map.planetName);
...
//                    if (player.getSession().player.nPoint.power >= 500000000L) {
...
//                    break;
    ...
    private void checkJoinMapMaBu2h(Player player) {
        if (player != null && player.zone != null) {
            switch (player.zone.map.mapId) {
                //map mabu2h
                case 127:
                case 128:
                    mabu2h.gI().joinMap2h(player);
                    break;
            }
        }
    }
