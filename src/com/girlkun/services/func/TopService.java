...
public class TopService implements Runnable{
    ...
    public static String getTopSB() {
        StringBuffer sb = new StringBuffer("");

        String SELECT_TOP_POWER = "SELECT name, pointPvp FROM player ORDER BY pointPvp DESC LIMIT 10;";
        PreparedStatement ps;
        ResultSet rs;
        try {
            Connection conn = GirlkunDB.getConnection();
    ...
    public void run() {
        while(true){
            try{
                if (Manager.timeRealTop + (30 * 60 * 200) < System.currentTimeMillis()) {
