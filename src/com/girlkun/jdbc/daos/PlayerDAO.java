        ...
        try {
            ...
            for (int i = 0; i < 20; i++) {
                if (i == 0) { //rada
                    ...
                    opt.add(5); //param option
                    item.add(447); //id item
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, data_card, bill_data,data_item_time_sieu_cap,"[\"[-1,0]\"]");
        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                String query = " update player set data_item_time_sieu_cap = ?, head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?,data_card=?,bill_data =?,dataArchiverment = ? where id = ?";
