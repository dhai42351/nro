...
//                    break;
                ...
                case -66:
                    ...
                    if (effId == 25 && SummonDragon.gI().playerSummonShenron.zone.map.mapId >14) {
                        idT = 50; // id eff rong muon thay doi ( hien tai la rong xuong) 
                    }
                    if (effId == 25 && GoiRongXuong.gI().playerRongXuong.zone.map.mapId == 5) {
                        idT = 51; // id eff rong muon thay doi ( hien tai la rong xuong) 
                    }

                    if (effId == 25 && SummonDragon.gI().playerSummonShenron.zone.map.mapId == 182) {
                        idT = 51; // id eff rong muon thay doi ( hien tai la rong xuong) 
                    }
...
//                                Service.gI().sendThongBao(player, "Đang bảo trì...");
    ...
    public void sendInfo(MySession session) {
        ...
        if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
            if ( player.getSession().version == 30){
                player.playerTask.taskMain.index = 2;
                TaskService.gI().sendNextTaskMain(player);
                player.zone = MapService.gI().getMapCanJoin(player, player.gender + 21,0);
                player.location.y = 336;
                        }
            NpcService.gI().createTutorial(player, -1,
                    "Chào mừng " + player.name + " đến với Ngọc rồng NIGHT \n"
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "Chào mừng \nbạn đến với Ngọc rồng NIGHT !!");
    }
